...
if [ $# -lt 1  ]
then
	echo -e "Usage: $0 --validate --Remediate --CIS=<CIS NUMBER>"
	echo -e "--Remediate\t\t\t\tUse this option for CIS Remediation"
	echo -e "--validate\t\t\t\tUse this option for CIS Validation Tests"
	echo -e "--CIS=<CIS NUMBER>\t\t\tUse this option for individual CIS benchmark"
fi
for arg in $@
do
	if [ $arg = "--validate" ]
	then
		echo "Option is --Validate"
	elif [ $arg = "--Remediate" ]
	then
		echo "You are running the Script for Remediation"
